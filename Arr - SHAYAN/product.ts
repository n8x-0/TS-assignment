...
type Product = { name: string, price: number, inventory: { stock: number, colorOptions: string[] } }
...
const changeColor = (product: string, color: string, price: number) => {
...
if (ques.product) {
    let selectedProduct: any = products.find(x => x.name === ques.product)
